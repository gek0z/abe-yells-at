...
/**
 * Encode a single canvas frame to WebP data in Node.js via @napi-rs/canvas.
 * Returns the raw WebP bytes (with RIFF wrapper intact — the muxer will strip it).
 */
export async function encodeWebPFrameNode(canvas: {
...
/**
 * Encode a single canvas frame to WebP data in the browser.
 * Uses OffscreenCanvas.convertToBlob or HTMLCanvasElement.toBlob.
 */
export async function encodeWebPFrameBrowser(
